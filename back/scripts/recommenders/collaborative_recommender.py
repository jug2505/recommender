...
class CollaborativeRecommender(BaseRecommender):
    ...
    def recommend_items_by_ratings(self, user_id, users_items, num=10):
        ...
        movie_ids = {movie['movie_id']: movie['rating'] for movie in users_items}
    ...
    def predict_score(self, user_id, item_id):
        user_items = Rating.objects.filter(user_id=user_id).exclude(movie_id=item_id).order_by('-rating')[:100]
    ...
    def predict_score_by_ratings(self, item_id, movie_ids):
        top = Decimal(0.0)
        bottom = Decimal(0.0)
        candidate_items = Similarity.objects.filter(source__in=movie_ids.keys()).exclude(source=item_id).filter(target=item_id)
